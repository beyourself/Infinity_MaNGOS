...
function top100($realmid, &$sqlr, &$sqlc)
{
  ...
  $result = $sqlc->query('SELECT count(*) FROM character_stats');
  ...
  $result = $sqlc->query('SELECT guid, name, race, class, gender, level, totaltime, online, money,
    ...
    melee_mainmindmg AS min_dmg,
    melee_mainmaxdmg AS max_dmg,
    ranged_mindmg AS min_ranged_dmg,
    ranged_maxdmg AS max_ranged_dmg,
    CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(equipmentCache, " ", '.(CHAR_DATA_OFFSET_EXPERTISE+1).'),         " ", -1) AS UNSIGNED) AS expertise,
    ...
    melee_hitrating AS melee_hit,
    ranged_hitrating AS range_hit,
    spell_hitrating AS spell_hit,
    totalHonorPoints AS honor,
    totalKills AS kills,
    arenaPoints AS arena
    FROM character_stats ORDER BY '.$order_by.' '.$order_dir.' LIMIT '.$start.', '.$itemperpage.'');
  ...
  while($char = $sqlc->fetch_assoc($result))
  {
    ...
    {
      $output .= '
                <td align="left"><img src="img/ranks/rank'.char_get_pvp_rank_id($char['honor'], char_get_side_id($char['race'])).'.gif" alt=""></img> '.char_get_pvp_rank_name($char['total_honor'], char_get_side_id($char['race'])).'</td>
                <td>'.$char['honor'].'</td>
                <td>'.$char['kills'].'</td>
                <td>'.$char['arena'].'</td>';
    }
