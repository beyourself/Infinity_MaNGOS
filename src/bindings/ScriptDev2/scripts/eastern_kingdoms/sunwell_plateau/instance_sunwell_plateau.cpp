...
void instance_sunwell_plateau::OnPlayerEnter(Player* pPlayer)
{
    ...
    pPlayer->SummonCreature(NPC_FELMYST, aMadrigosaLoc[0].x, aMadrigosaLoc[0].y, aMadrigosaLoc[0].z, aMadrigosaLoc[0].o, TEMPSUMMON_DEAD_DESPAWN, 0);
}
...
void instance_sunwell_plateau::JustDidDialogueStep(int32 iEntry)
{
    switch (iEntry)
    {
        case NPC_KALECGOS_MADRIGOSA:
            ...
            {
                if (Creature* pKalec = pTrigger->SummonCreature(NPC_KALECGOS_MADRIGOSA, aKalecLoc[0].x, aKalecLoc[0].y, aKalecLoc[0].z, aKalecLoc[0].o, TEMPSUMMON_CORPSE_DESPAWN, 0))
                {
                    pKalec->SetWalk(false);
                    pKalec->SetLevitate(true);
                    pKalec->GetMotionMaster()->MovePoint(0, aKalecLoc[1].x, aKalecLoc[1].y, aKalecLoc[1].z, false);
                }
            }
            break;
        case NPC_FELMYST:
            if (Creature* pKalec = GetSingleCreatureFromStorage(NPC_KALECGOS_MADRIGOSA))
                pKalec->GetMotionMaster()->MovePoint(0, aKalecLoc[2].x, aKalecLoc[2].y, aKalecLoc[2].z, false);
            break;
        ...
        case NPC_BRUTALLUS:
            if (Creature* pKalec = GetSingleCreatureFromStorage(NPC_KALECGOS_MADRIGOSA))
            {
                ...
                pKalec->GetMotionMaster()->MovePoint(0, aKalecLoc[3].x, aKalecLoc[3].y, aKalecLoc[3].z, false);
            }
            break;
    }
}
