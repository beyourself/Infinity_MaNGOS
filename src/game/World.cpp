...
void World::SetInitialWorldSettings()
{
    ...
    {
        sLog.outError("Correct *.map files not found in path '%smaps' or *.vmtree/*.vmtile files in '%svmaps'. Please place *.map and vmap files in appropriate directories or correct the DataDir value in the world.conf file.",m_dataPath.c_str(),m_dataPath.c_str());
        Log::WaitBeforeContinueIfNeed();
        exit(1);
    }
...
bool World::configNoReload(bool reload, eConfigUInt32Values index, char const* fieldname, uint32 defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError("%s option can't be changed at world.conf reload, using current value (%u).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigInt32Values index, char const* fieldname, int32 defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError("%s option can't be changed at world.conf reload, using current value (%i).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigFloatValues index, char const* fieldname, float defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError("%s option can't be changed at world.conf reload, using current value (%f).", fieldname, getConfig(index));
...
bool World::configNoReload(bool reload, eConfigBoolValues index, char const* fieldname, bool defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError("%s option can't be changed at world.conf reload, using current value (%s).", fieldname, getConfig(index) ? "'true'" : "'false'");
