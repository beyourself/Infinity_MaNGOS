    ...
    // Loading levels data
    {
        ...
        do
        {
            ...
            if(current_level > maxLevel)
            {
                ...
                {
                    DETAIL_LOG("Unused (> MaxPlayerLevel in world.conf) level %u in `pet_levelstats` table, ignoring.",current_level);
    ...
    // Loading levels data (class/race dependent)
    {
        ...
        do
        {
            ...
            if (current_level > sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    DETAIL_LOG("Unused (> MaxPlayerLevel in world.conf) level %u in `player_levelstats` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
    ...
    // Loading xp per level data
    {
        ...
        do
        {
            ...
            if (current_level >= sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    DETAIL_LOG("Unused (> MaxPlayerLevel in world.conf) level %u in `player_xp_for_levels` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
